...
def scale_landmarks_to_original(
    ...
    for landmark in landmarks:
        if len(landmark) < 2:
            continue
        x = max(0.0, min((float(landmark[0]) - pad_w) / scale, float(orig_w)))
        y = max(0.0, min((float(landmark[1]) - pad_h) / scale, float(orig_h)))
        if len(landmark) > 2:
            scaled.append([x, y, float(landmark[2])])
        else:
            scaled.append([x, y])
    return scaled
...
def scale_detections_to_original(detections: List[dict], orig_h: int, orig_w: int, scale: float, pad_w: int, pad_h: int) -> List[dict]:
    ...
    for det in detections:
        ...
        if det.get('landmarks'):
            det['landmarks'] = scale_landmarks_to_original(det['landmarks'], orig_h, orig_w, scale, pad_w, pad_h)
...
class HailoPythonInferenceEngine:
    ...
    def _run_python_head(self, dequantized_results: Dict, conf_threshold: float) -> List[dict]:
        ...
        for name, data in dequantized_results.items():
            ...
            if len(shape) != 4 or shape[0] != 1 or shape[1] != shape[2] or shape[1] not in (80, 40, 20):
                continue

            grid_dim = shape[1]
            channels = shape[3]
            if channels == 4:
                tensors[f'reg_{grid_dim}'] = data
            elif channels == 15:
                tensors[f'kpt_{grid_dim}'] = data
            else:
                tensors[f'cls_{grid_dim}'] = data
        ...
        if not tensors:
            ...
            for name, data in dequantized_results.items():
                ...
                if channels == 4:
                    ...
                    num_classes = channels
            if len(reg_tensors) == 3 and len(cls_tensors) == 3:
                for grid_dim in (80, 40, 20):
                    tensors[f'reg_{grid_dim}'] = reg_tensors[grid_dim]
                    tensors[f'cls_{grid_dim}'] = cls_tensors[grid_dim]
        ...
        has_landmarks = all(f'kpt_{grid_dim}' in tensors for grid_dim in (80, 40, 20))
        if num_classes is None:
            num_classes = 1
        ...
        for scale_idx in range(len(STRIDES)):
            ...
            kpt_data = tensors.get(f'kpt_{grid_dim}')
            kpt_flat = kpt_data[0].reshape(-1, 15) if kpt_data is not None else None
            ...
            landmarks = None
            if has_landmarks and kpt_flat is not None:
                raw_kpts = kpt_flat[indices].reshape(-1, 5, 3)
                landmarks = np.empty_like(raw_kpts, dtype=np.float32)
                landmarks[:, :, 0] = (raw_kpts[:, :, 0] + cols[:, None] + 0.5) * stride
                landmarks[:, :, 1] = (raw_kpts[:, :, 1] + rows[:, None] + 0.5) * stride
                landmarks[:, :, 2] = sigmoid(raw_kpts[:, :, 2])
            ...
            for j in range(len(indices)):
                ...
                if landmarks is not None:
                    detection['landmarks'] = [
                        [round(float(point[0]), 2), round(float(point[1]), 2), round(float(point[2]), 4)]
